...
func servidorTeste(t *testing.T) (*Servidor, *coleta.Coletor) {
	t.Helper()
	c := coleta.NovoColetor(coleta.FontesDeTeste(t, map[string]string{
		"/proc/stat":    "cpu  0 0 0 1000 0 0 0 0 0 0\n",
		"/proc/meminfo": "MemTotal: 1000 kB\nMemAvailable: 500 kB\n",
	}), 5*time.Second, versao)
	c.ColetarAgora()
...
func TestMetricsDevolveJSONValido(t *testing.T) {
	...
	var s2 metricas.Snapshot
	if err := json.Unmarshal(w.Body.Bytes(), &s2); err != nil {
		t.Fatalf("resposta nao e um metricas.Snapshot valido: %v", err)
	}
	if s2.V != versao {
