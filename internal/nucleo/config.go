...
func (c Config) ComoBruto() map[string]any {
	...
	// Preserva o que o arquivo ja tinha e este codigo nao conhece - a arvore
	// do smart, sobretudo. Sem isto, salvar pela janela apagaria em silencio
	// os limiares de disco que alguem ajustou a mao.
	if antigos, ok := c.Bruto["alertas"].(map[string]any); ok {
		for k, v := range antigos {
			alertas[k] = v
		}
	}
